...
public class GuildUserRemoveHandlerTests
{
    ...
    private readonly Mock<ILogger<GuildUserRemoveHandler>>   _logger       = new();
    private readonly GuildUserRemoveHandler                  _sut;

    private const ulong UserId  = 511624657162731533UL;
    private const ulong GuildId = 796438478925725796UL;

    public GuildUserRemoveHandlerTests()
    {
        ...
        _sut = new GuildUserRemoveHandler(_scopeFactory.Object, _logger.Object);
    }
    ...
    [InlineData(true)]
    [InlineData(false)]
    public async Task HandleAsync_NotifiesTheRemovedUser(bool loggerEnabled)
    {
        _logger.Setup(l => l.IsEnabled(It.IsAny<LogLevel>())).Returns(loggerEnabled);
        var arg = NetCordTestHelpers.MakeGuildUserRemoveEventArgs(GuildId, UserId);
