...
public class GuildMembershipRepository(RaidOpsDbContext context) : IGuildMembershipRepository
{
    ...
    /// <summary>
    /// Returns all roster memberships for the given guild branch with character, class, branch,
    /// realm, user-curated raid spec and expansion state (for level) navigation data.
    /// </summary>
    public async Task<List<GuildMembership>> GetByGuildBranchIdAsync(int guildBranchId, CancellationToken cancellationToken = default)
        => await context.GuildMemberships
            ...
            .Where(m => m.GuildBranchId == guildBranchId && m.Character.IsActiveInRaidOps)
            .AsNoTracking()
            .ToListAsync(cancellationToken);
