...
/// the updated <see cref="CharacterDto"/>. Uses an app-level token (client credentials, public
/// data only) scoped to the character's own <see cref="Realm.Region"/> — independent of which
/// BNet accounts the user currently has linked.
/// </summary>
    ...
    : ICommandHandlerAsync<ResyncCharacterCommand>
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var region = character.Realm.Region;
        ...
        try
        {
            appToken = await bnetApiService.GetAppTokenAsync(region, cancellationToken);
        }
        catch (HttpRequestException ex)
        {
            logger.LogError(ex,
                ...
                character.Id, name, command.UserDiscordId, region);
            return Result<CommandResponse>.Fail(ResponseDetail.BnetApiError);
        }

        var detailTask = bnetApiService.GetCharacterAsync(appToken, region, profileNamespace, realmSlug, name, cancellationToken);
        var mediaTask  = bnetApiService.GetCharacterMediaAsync(appToken, region, profileNamespace, realmSlug, name, cancellationToken);
        var specsTask  = bnetApiService.GetCharacterSpecializationsAsync(appToken, region, profileNamespace, realmSlug, name, cancellationToken);
