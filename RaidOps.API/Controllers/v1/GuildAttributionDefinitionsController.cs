...
{
    /// <summary>Returns the guild's raid-attribution template, ordered for display.</summary>
    [HttpGet("{guildId}/attribution-definitions")]
    public async Task<IActionResult> GetDefinitions(string guildId, CancellationToken cancellationToken)
    {
        ...
        var result = await QueryDispatcher.DispatchAsync<GetGuildAttributionDefinitionsQuery, List<GuildAttributionDefinitionResponse>>(
            new GetGuildAttributionDefinitionsQuery { GuildId = guildId, RequesterDiscordId = discordId },
            cancellationToken);
